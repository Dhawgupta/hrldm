...
def run(option = option, episodes = 100000, no_slots = NO_SLOTS,meta_state_size = META_STATE_SIZE,meta_option_size = META_OPTION_SIZE,controller_state_size = CONTROLLER_STATE_SIZE,controller_action_size = CONTROLLER_ACTION_SIZE):
    ...
    filename = "{}_{}_HiddenLayers_{}_Dropout_{}_LearningRate_{}_Gamma_{}_Activation_{}_Episode_{}_option_{}.h5".format(filename, a ,str(agent.hiddenLayers), str(agent.dropout) , str(agent.learning_rate), str(agent.gamma), agent.activation, str(EPISODES), option)
...
def main():
    ...
    run(option= args.option, episodes=args.epsiodes)
    return 0
