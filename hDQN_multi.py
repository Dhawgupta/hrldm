...
NO_SLOTS = 8
INTENT_STATE_SIZE = 5
META_STATE_SIZE = 5 + 8 # each 5 slots tells us the intent that are currently requried to be served by the agent
META_OPTION_SIZE = 5 + 1 # pickig each individual intent, and also pincking th aciton of asking the user a question for the next intent
CONTROLLER_STATE_SIZE = META_OPTION_SIZE + NO_SLOTS # the itnent that is to be served and the lstos that ar ethere
# in the CONTROLLER_STATE_SZEI , there will be an extra option for teh meta-option_size which will be responsibel for taking the action to ask the user a question
...
default_nodes = [CONTROLLER_STATE_SIZE, 30, 30, 30,
                 CONTROLLER_ACTION_SIZE]  # the actions  is   8[each slot] + 3[hybrod action] + 8[reask eacj slot] + 1[termiate the dioalogue-] = 20
...
class hDQN:
    def __init__(self, meta_layers=default_meta_layers, meta_inits=default_meta_inits,
                 ...
                 meta_epsilon=default_meta_epsilon, epsilon=default_epsilon, actor_epsilon=default_actor_epsilon,
                 tau=default_tau,
    ...
    def create_meta_controller(self):  # This is theta controller
        ...
        for layer, init, node, activation in list(
                zip(self.meta_layers, self.meta_inits, self.meta_nodes, self.meta_activations))[1:]:
    ...
    def _update(self):
        # print("_Update")
        exps = [random.choice(self.memory) for _ in
                range(self.n_samples)]  # sample n_samples from the controller memory
        state_vectors = np.squeeze(np.asarray([np.concatenate([exp.state, exp.goal]) for exp in
                                               exps]))  # for each experince exp contains the state, the goal, action, reward, next state and the same goal, hence we need to make the state vector by concat the s and g vectors
        next_state_vectors = np.squeeze(np.asarray([np.concatenate([exp.next_state, exp.goal]) for exp in
                                                    exps]))  # the same and the squeeze operator converts the 2 d array into a 1 d Array where ever there is single dimension
        ...
        try:
            reward_vectors = self.actor.predict(state_vectors,
                                                verbose=0)  # accumulate the Q(s,a) functions for each action
        ...
        try:
            next_state_reward_vectors = self.target_actor.predict(next_state_vectors,
                                                                  verbose=0)  # Accumulate the Q(s',a') for each action in the s' state
        except Exception as e:
        ...
        for i, exp in enumerate(exps):
            reward_vectors[i][
                exp.action] = exp.reward  # Put the reward for action if the next state was terminal as the reward attained for tha state, i.e. this will only happen if after this state the epsiode or controller terminates
            if not exp.done:
        ...
        for i in range(len(actor_weights)):
            actor_target_weights[i] = self.target_tau * actor_weights[i] + (1 - self.target_tau) * actor_target_weights[
                i]
    ...
    def _update_meta(self):
        ...
        if 0 < len(self.meta_memory):
            ...
            for i in range(len(meta_weights)):
                meta_target_weights[i] = self.target_tau * meta_weights[i] + (1 - self.target_tau) * \
                                         meta_target_weights[i]
